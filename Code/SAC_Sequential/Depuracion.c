...
int main(int argc, char* argv[]){
	...
	uint32_t size;
	size = 1016732543;//504281892;//1016732543;//1083792832;// (uint32_t) atol(argv[1]);
	//size = 150000;
//	printf("size= %ld\n",size);	fflush(stdout);
	uint8_t *cadena = (uint8_t *)calloc(size+1, sizeof(uint8_t));
	if (cadena  == NULL) {	/* Error al intentar reservar memoria */		printf("error cadena\n");	}
	uint32_t *Indexes = (uint32_t*) calloc(size+1, sizeof(uint32_t));//uint32
	if (Indexes  == NULL) {	/* Error al intentar reservar memoria */		printf("error Indexes\n");	}
	...
	gen_data(cadena, size);
		...
		#endif
	ofstream myfile;
	myfile.open ("SAC SEQ Experiments Oct_18.txt",ios::app);
	myfile << " --------------------New Round of experiments Sequential----------------------\n" << endl;
	myfile.close();	
	printf ("%d nthreads , tamaño %u\n", 1, size); fflush(stdout);
	...
	cout << " 1 Thread,  Milliseconds in SEQ_ONLY RadixSAC:"<< duration_cast<milliseconds>(stopLocal-startLocal).count() << "Elements"<< size << ".\n" << endl;
	myfile.open ("SAC Experiments Oct_18.txt",ios::app);
	myfile << "1 Thread,  Milliseconds in SEQ_ONLY RadixSAC:"<< duration_cast<milliseconds>(stopLocal-startLocal).count() << "Elements"<< size << ".\n" << endl;
	myfile.close();	
